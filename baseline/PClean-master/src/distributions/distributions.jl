...
function resample_value!(p::IndexedParameter)
  @assert isa(p, IndexedParameter)
  @assert isa(p.parameters, Dict)

  # 获取参数键的排序
  param_keys = sort(collect(keys(p.parameters)))
  print("1")
  println("Parameter keys: ", param_keys)
  for key in param_keys
    parameter = p.parameters[key]
    println("Resampling for key $key")
    resample_value!(parameter)
  end
end
